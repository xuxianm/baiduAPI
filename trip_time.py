    ...
    for j in range(4):
        ...
        for item in start_data['results']:
            ...
            for item in end_data['results']:
                ...
                trans_url = 'http://api.map.baidu.com/direction/v2/transit?origin=' + start_lat +',' + start_lng + '&destination=' + end_lat +',' + end_lng + '&ak=IyBt2aQhVm6fkjC0iqFrRx1RBRetd75v'
                print(trans_url)
                trans_html = requests.get(trans_url)
                trans_data = trans_html.json()
                #print(trans_data)
                #print(type(trans_data))
                if(trans_data['result']['routes']):
                    #路线数
                    n = 0
                    style = '公交'
                    for n in range(5):
                        try:
                            if(trans_data['result']['routes'][n]):
                                a =trans_data['result']['routes'][n]
                                distance = a['distance']
                                duration = a['duration']
                                time = time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S').split()[1]
                                value =(start_name, end_name, start_area, end_area,n,distance,duration,style,time)
                                print(value)
                                sql = 'insert into trip_time(origin,destination,origin_area,destination_area,route,distance,duration,style,time) values{}'.format(value)
                                cusor.execute(sql)
                                conn.commit()
